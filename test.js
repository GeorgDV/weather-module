...
async function test(){
    ...
    let londonWeather = await WeatherClient.getWeather('london', 0);
    ...
    */
    ...
    let cachedWeatherInfo = await WeatherClient.getWeatherWithCache('london', 2);
    console.log('London weather info with cache: ', cachedWeatherInfo);

    let cachedTempInfo = await WeatherClient.getTempWithCache('london', 1);
